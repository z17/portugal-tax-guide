# Portugal Tax Guide

В этом проекте собирается основная информация и гайды по самым частым типичным сценариям. Ограничения и применимость описаны в каждой статье. По возможности приведены ссылки на первоисточники. 

Всю информацию тут стоит перепроверять и использовать на свой страх и риск, она не проверена налоговым экспертом или консультантом.

Если вы нашли неточность — сообщите, пожалуйста. Если есть силы доработать любую из статей или добавить новую — буду рад, исходники на [github](https://github.com/z17/portugal-tax-guide/). 

## Разделы
- [Обзор](?page=overview)
- ИП / Atividade independente
  - [Общая информация](?page=atividade-independente-info)
  - [Что такое и как выпускать Fatura-Recibo](?page=fatura-recibo)
  - [Как и зачем размечать траты по категориям](?page=e-fatura-marking)
- IRS
  - [Общая информация](?page=irs) 
  - Как и когда подавать Declaração de Rendimentos Modelo 3 de IRS
- IVA
  - Общая информация
  - [Что такое и как подать Declaração Recapitulativa do IVA](?page=recapulativa-iva)
  - Что такое и как подать Declaração periódica do IVA
- Социальный налог
  - [Общая информация](?page=seg-social)
  - [Как и когда подавать Declaração trimestral à Segurança Social](?page=seg-social-trimestral)
  - [Как и когда платить](?page=seg-social-payment)

